...
datafd(BTS) is a statistical agency that is a part of the Research and Innovative 
...
We begin by loading the `nycflights` data frame. Type the following 
in your console
to load the data:
...
```{r sfo-feb-flights}
sfo_fe_flights <- nycflights %>% filter(dest=="SFO", month==2)
dim(sfo_fe_flights)
```
...
ggplot(data = nycflights, aes(x = arr_delay)) + geom_histogram(binwidth=15)
...
sfo_feb_flights %>% group_by(carrier) %>% summarise(iqr_ad=IQR(arr_delay))
...
nycflights%>%
summarise(dd_otarr=sum(dep_type=="delayed" & arr_type=="on time")/sum(dep_type=="delayed"))
```